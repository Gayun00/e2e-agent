...
이 기능은 Page Object Model(POM) 패턴을 사용하여 Playwright E2E 테스트를 자동으로 생성하는 AI Agent를 만드는 것을 목표로 합니다. 최신 구조에서는 CrewAI가 Anthropic Claude와 Playwright MCP 도구를 orchestration 하여, 에이전트가 스스로 브라우저를 탐색하고 사용자와 대화하면서 선택자와 메서드를 채워 넣습니다. Agent는 테스트 시나리오를 분석하고, 페이지 객체를 생성하며, 요소 선택자를 지능적으로 선택하고, 테스트 동작을 생성하며, 완전한 테스트 시나리오를 구성합니다. 또한 페이지 구조와 도메인 지식에 대한 문서를 유지하여 테스트 생성의 정확도를 향상시킵니다.
...
1. WHEN Agent가 MCP를 사용해 선택자를 탐색하면 THEN 해당 근거(스크린샷, DOM 스니펫, 로그)를 포함해 개발자 확인을 요청해야 합니다
2. WHEN Agent가 특정 메서드 구현을 제안할 때 THEN “승인/수정/건너뛰기” 선택지가 즉시 제공되어야 합니다
3. IF 개발자가 수동 selector나 코드 스니펫을 입력하면 THEN Agent는 동일한 MCP 세션을 이용해 재검증 후 결과를 공유해야 합니다
4. WHEN Agent가 모호함을 만나거나 MCP 실패가 반복되면 THEN 가정을 하지 않고 명확한 질문과 대안을 제시해야 합니다
5. WHEN 페이지/메서드/플로우 단위 단계가 완료되면 THEN Agent는 다음 단계로 넘어가기 전에 명시적 승인을 받아야 합니다
6. WHEN 개발자가 승인한 내용은 세션 state에 저장되어, 재시작 시 중복 질문 없이 바로 이어갈 수 있어야 합니다

### 요구사항 14: CrewAI 기반 에이전트 오케스트레이션

**사용자 스토리:** 플랫폼 엔지니어로서, LLM 에이전트가 MCP 도구를 직접 호출하고, 단계별 상태를 유지하며, CLI는 단순히 대화·로그를 중계하기만 하도록 만들고 싶습니다.

#### 인수 기준

1. WHEN Agent 세션이 시작되면 THEN CrewAI는 Anthropic LLM, MCP tools, 파일 시스템 등을 포함한 역할/작업 구성을 자동으로 로드해야 합니다
2. WHEN Agent가 MCP 도구 호출이 필요하다고 판단하면 THEN CLI 개입 없이 CrewAI tool 인터페이스를 통해 `browser_*` 호출을 수행해야 합니다
3. WHEN MCP 세션이 생성되면 THEN 세션 ID와 브라우저 컨텍스트는 Agent state에 저장되어, 메서드별 실행 간 재사용돼야 합니다
4. IF Agent 세션이 중단되면 THEN 마지막 체크포인트(state + 승인 내역)를 활용해 동일 브라우저 컨텍스트 또는 새로운 컨텍스트로 재개할 수 있어야 합니다
5. WHEN CLI 사용자가 명령을 실행하면 THEN 실제 작업은 CrewAI 에이전트 대화로 전환되고, CLI는 대화 로그/승인 UI만 제공해야 합니다
6. WHEN 여러 도구(MCP, 파일 시스템, Langfuse 등)를 사용할 때 THEN Agent는 호출 내역을 콘솔/로그에 순차적으로 남겨 추적 가능해야 합니다
